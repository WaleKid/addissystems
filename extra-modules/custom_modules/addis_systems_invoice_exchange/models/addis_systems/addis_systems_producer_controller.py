...
async def invoice_producer(invoice_id):
        ...
        invoice_line.append(
            {
                ...
                "Product_desc": str(
                    line.product_id.product_tmpl_id.description_sale or ""
                ),
    ...
    try:
        ...
        if client:
            invoice_produced = await _invoice_mor_producer(
                invoice_id, client, invoice_data
            )
            if invoice_produced:
                invoice_acknowledged_consumed = await _invoice_mor_consumer(
                    invoice_id, client
                )
...
async def _invoice_mor_producer(invoice_id, client, invoice_data):
    ...
    try:
        own_producer = client.create_producer(
            "persistent://"
            + str(invoice_id.env.company.name).replace(" ", "").lower()
            + "/invoice/sales"
        )
        ...
        if own_producer and mor_producer:
            ...
            mor_producer.send(
                bytes_writer.getvalue(), properties={"key": "invoice_key"}
            )
...
async def _invoice_mor_consumer(invoice_id, client):
    ...
    consumer_conf = {
        "subscription_name": str(invoice_id.env.company.name).replace(" ", "").lower()
    }
    ...
    try:
        ...
        _logger.warning(
            "Addis Systems topic subscription couldn't be achieved %s, %s",
            topic_name,
            consumer_conf,
        )
    finally:
        if consumer:
            ...
            is_e_invoice = await check_partner_electronic_invoice_user(
            ...
            invoice_id.invoice_status = (
                "registered" if datajson["Inv_Status"] == "Registered" else "failed"
            )
...
async def invoice_log_tracking_producer_consume(invoice_id):
    ...
    try:
        ...
        if client:
            try:
                log_producer = client.create_producer(
                    "persistent://pulsar/default/LogTracking"
                )
            except Exception as e:
                _logger.warning("%s:Addis Systems couldn't create producers!", e)
            finally:
                if log_producer:
                    log_data = {
                        "InvoiceCreatedDate": str(fields.Datetime.now()),
                        "InvoiceID": str(invoice_id.name or None),
                        "CompanyName": str(invoice_id.env.company.name)
                        .replace(" ", "")
                        .lower(),
                        "ErrorResponse": "",
                        "ErrorStatus": {
                            "sentStatus": {"Sent_INV": "", "Sent_INV_AT": ""},
                            "received_Status": {
                                "Received_INV": "",
                                "Received_INV_AT": "",
                            },
                            "Ack_gen_Status": {"Ack_Gen_INV": "", "Ack_Gen_INV_AT": ""},
                            "Ack_save_Status": {
                                "Ack_Save_INV": "",
                                "Ack_Save_INV_AT": "",
                            },
...
async def refund_producer(refund_id):
        ...
        refund_line.append(
            {
                ...
                "invoicing_policy": str(
                    line.product_id.product_tmpl_id.invoice_policy or ""
                ),
                "Product_desc": str(
                    line.product_id.product_tmpl_id.description_sale or ""
                ),
                ...
                "product_type": str(
                    line.product_id.product_tmpl_id.detailed_type or ""
                ),
                "product_category": str(
                    line.product_id.product_tmpl_id.categ_id.name or ""
                ),
    ...
    try:
        ...
        if client:
            ...
            if refund_produced:
                refund_acknowledged_consumed = await _refund_mor_consumer(
                    refund_id, client
                )
...
async def _refund_mor_producer(refund_id, client, refund_data):
    ...
    try:
        own_producer = client.create_producer(
            "persistent://"
            + str(refund_id.env.company.name).replace(" ", "").lower()
            + "/invoice/refund"
        )
...
async def _refund_mor_consumer(refund_id, client):
    ...
    consumer_conf = {
        "subscription_name": str(refund_id.env.company.name).replace(" ", "").lower()
    }
    ...
    try:
        ...
        _logger.warning(
            "Addis Systems topic subscription couldn't be achieved",
            topic_name,
            consumer_conf,
        )
    finally:
            ...
            try:
                ...
                if log_producer:
                    log_data = {
                        "InvoiceCreatedDate": str(fields.Datetime.now()),
                        "InvoiceID": str(refund_id.name or None),
                        "CompanyName": str(refund_id.env.company.name)
                        .replace(" ", "")
                        .lower(),
                        "ErrorResponse": "",
                        "ErrorStatus": {
