...
class AddisSystemsThemeIrHttp(models.AbstractModel):
    ...
    def session_info(self):
        ...
        result["pager_autoload_interval"] = int(self.env["ir.config_parameter"].sudo().get_param("addis_systems_theme.autoload", default=30000))
        return result
...
class AddisSystemsThemeResConfigSettings(models.TransientModel):
    ...
    def action_reset_theme_assets(self):
        self.env["web_editor.assets"].reset_asset("/addis_systems_theme/static/src/colors.scss", "web._assets_primary_variables")
    ...
    def set_values(self):
        ...
        variables = [
            ...
            "as-menu-color",
            "as-appbar-color",
            "as-appbar-background",
        ]
        colors = self.env["web_editor.assets"].get_theme_variables_values("/addis_systems_theme/static/src/colors.scss", "web._assets_primary_variables", variables)
        colors_changed = [
            self.theme_color_brand != colors["o-brand-odoo"],
            self.theme_color_primary != colors["o-brand-primary"],
            self.theme_color_menu != colors["as-menu-color"],
            self.theme_color_appbar_color != colors["as-appbar-color"],
        ...
        if any(colors_changed):
            variables = [
                ...
                {
                    "name": "as-appbar-color",
                    "value": self.theme_color_appbar_color or "#dee2e6",
                },
                {
                    "name": "as-appbar-background",
                    "value": self.theme_color_appbar_background or "#000000",
                },
            ]
            self.env["web_editor.assets"].replace_theme_variables_values("/addis_systems_theme/static/src/colors.scss", "web._assets_primary_variables", variables)
        return res
...
class ResUsers(models.Model):
    ...
    sidebar_type = fields.Selection(selection=[("invisible", "Invisible"), ("small", "Small"), ("large", "Large")], string="Sidebar Type", default="invisible", required=True)
