...
def addis_systems_lead_digest_async(as_lead_env, thread_name):
    ...
    def addis_systems_lead_pulsar_waiter(as_lead_env):
        ...
        try:
            lead_client = pulsar.Client("pulsar://192.168.100.208:6650")
    ...
    def crm_lead_create(as_lead_env, lead_data_json, account_info):
        ...
        with as_lead_env.env.registry.cursor() as new_cr:
            ...
            company = None
            new_partner = None
            country_id = env['res.country'].search([('name', '=', country)])

            # Company Check Up

            if company_exist := env['res.partner'].search([('name', '=', str(company_name)), ('is_company', '=', True)], limit=1):
                ...
                company = env['res.partner'].create(company_vals)

            # Partner Check Up

            partner_id = env['res.partner'].search(['|', ('name', '=', company_name), ('vat', '=', tin), ('is_company', '=', False)], limit=1)

            if not partner_id:
                new_partner = env['res.partner'].create({
                    'name': str(f'{first_name} {last_name}'),
                    'company_type': 'person',
                    'parent_id': company.id or None,
                    'function': title or None,
                    'vat': tin or None,
                    'city': city or None,
                    'country_id': country_id.id if country_id else None,
                    'phone': phone or None,
    ...
    addis_systems_lead_pulsar_waiter(as_lead_env)
